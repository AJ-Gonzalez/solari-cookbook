...
def _print_queue(conn, limit: int, statuses, eligible_only: bool) -> None:
    rows = db.ranked_rows(conn, statuses, eligible_only)
    if not rows:
...
def cmd_mark(args: argparse.Namespace) -> None:
    ...
    print(f"[{args.id}] {args.status}: {r['title']} — {r['company']}")
